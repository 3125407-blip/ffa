...
async function cmdLogin() {
    ...
    if (!player) {
        ...
        if (player.password !== pass) {
            document.getElementById('log').innerHTML = "店主：「おっと、パスワードが違うようだね。」";
            return;
        }
        myData = player;
...
function updateUI() {
    ...
    myData.inventory.bag.forEach(item => {
        bagHtml += `<span style="margin-right:5px; border:1px solid #555; padding:2px;">${item}<button onclick="cmdEquip('${item}')" style="padding:1px 4px; font-size:10px;">装備</button></span>`;
    });
...
async function cmdBattle() {
    ...
    for (let turn = 1; turn <= 200; turn++) {
        ...
        if (p.job === "アーチャー" && Math.random() < 0.3) {
            dmgMult = 1.2;
            battleLog += `<span style="color:#facc15;">[クリティカル！]</span> `;
        }
        if (p.job === "魔法使い" && Math.random() < 0.2) {
            let healAmt = Math.floor(p.lv / 2);
            p.hp = Math.min(Number(p.base_max_hp)*2, p.hp + healAmt);
        ...
        battleLog += `${enemy.n}の攻撃！ <span style="color:#ef4444;">${f(eDmg)}</span> のダメージを受けた！ (残りHP: ${f(p.hp)})<br>`;

        if (p.hp <= 0) { battleLog += `力尽きた...<br>`; break; }
    }
    ...
    if (isWin) {
        ...
        // ドロップ判定
        if (enemy.n === "ゴブリン" && Math.random() < 0.2) { p.inventory.bag.push("ナイフ"); battleLog += `<br>「ナイフ」GET!`; }
        if (enemy.n === "月の番人" && Math.random() < 0.2) { p.inventory.bag.push("天の羽衣"); battleLog += `<br>「天の羽衣」GET!`; }
        ...
        while (p.exp >= 100 + (p.lv * 50)) {
            ...
            battleLog += `<br><span style="color:#a855f7;"><b>LvUP! Lv${f(p.lv)}</b></span>`;
        }
    }

    // 保存
    await _supabase.from('players').update({
        hp: p.hp, base_max_hp: p.base_max_hp, lv: p.lv, exp: p.exp, gold: p.gold, inventory: p.inventory
    }).eq('name', p.name);
...
async function cmdRest() {
    ...
    document.getElementById('log').innerText = "宿屋で全快しました！";
    updateUI();
}

// 6. ショップ関連（全アイテム移植）
function openYourShop() {
    let html = `<h3>ーーー 武器屋 ーーー</h3><div style="text-align:left; background:#111; padding:10px;">`;
    const list = [
...
async function executePurchase(id) {
    const list = [
    ...
    if(myData.gold < item.p) return alert("お金が足りません。");
    ...
    if(id.startsWith("scroll_")){
        ...
        myData.lv += item.lv; myData.base_max_hp += (15 * item.lv);
    } else {
        myData.inventory.bag.push(item.n);
    }

    await _supabase.from('players').update({
        gold:myData.gold, job:myData.job, hp:myData.hp, lv:myData.lv, base_max_hp:myData.base_max_hp, inventory:myData.inventory
    }).eq('name', myData.name);
    updateUI();
    document.getElementById('log').innerText = `${item.n} を購入しました！`;
}
...
async function cmdRaidChallenge() {
    const now = Date.now();
    if(now - myData.last_raid < 100000) return alert("待機中です。");
    const { data: boss } = await _supabase.from('raid_boss').select('*').eq('id', 1).single();
...
async function refreshChat() {
    const { data } = await _supabase.from('chat').select('*').order('created_at', {ascending: false}).limit(15);
    let html = "";
